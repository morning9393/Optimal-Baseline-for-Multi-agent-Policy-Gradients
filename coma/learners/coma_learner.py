...
class COMALearner:
    def __init__(self, mac, scheme, logger, args):
        ...
        self.agent_scheduler = th.optim.lr_scheduler.ExponentialLR(self.agent_optimiser, gamma=0.99)
        self.critic_optimiser = RMSprop(params=self.critic_params, lr=args.critic_lr, alpha=args.optim_alpha,
                                        eps=args.optim_eps)

    # Optimal baseline --------- Muning
    def optimal_baseline(self, q, pi):
        """
        Calculating the optimal baseline
        :param q: (batch_size, n_action), Q values of all actions under the same states
        :param pi: (batch_size, n_action), Probability distributions of actions
        :return: The optimal baseline
        """
        M = th.norm(pi, dim=-1) ** 2 + 1
        M = M.unsqueeze(1)
        xweight = - 2 * pi + M
        enum = (pi * xweight * q).sum(-1)
        deno = (pi * xweight).sum(-1)
        return (enum / deno).detach() # (batch_size, )
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        # Optimal baseline -------- Muning
        if self.use_ob:
            pi = th.clamp(pi, min=0.0001, max=0.9999)  # a small biased to avoid deno = 0
            X = self.optimal_baseline(q_vals, pi)  # ob
        else:
            X = (pi * q_vals).sum(-1).detach()
